...
# ========== DATABASE ==========
conn = sqlite3.connect("configbot.db")
c = conn.cursor()
...
# ========== ON_READY (FIXED - reminders start here) ==========
@bot.event
async def on_ready():
    ...
    # Start reminders loop HERE (inside on_ready)
...
@bot.command()
@commands.has_permissions(administrator=True)
async def verifypanel(ctx, role: discord.Role):
    embed = discord.Embed(title="✅ Verification", description="Click below to verify", color=discord.Color.green())
